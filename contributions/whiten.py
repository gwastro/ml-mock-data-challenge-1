...
def whiten(strain, delta_t=1./2048., segment_duration=0.5,
    ...
    if strain.ndim==1:
        ...
        if psd is None:
            ...
            assert psd.ndim==1
            print('WARNING: Assuming PSD delta_f based on delta_t, length of PSD and EVEN length of original time series. Tread carefully!')
        ...
        inv_psd = 1./psd
        white_ts = (colored_ts.to_frequencyseries()*inv_psd**0.5).to_timeseries()
        ...
        white_segments = [whiten(sd_strain, delta_t=delta_t,
                ...
                low_frequency_cutoff=low_frequency_cutoff,
                psd=psd_1d, **kwargs) for sd_strain, psd_1d in zip(strain, psds_1d)]
...
def main():
    ...
    with h5py.File(args.inputfile, 'r') as infile, h5py.File(args.outputfile, 'x') as outfile:
        copy_attrs(infile, outfile)
        iterable1 = infile.items()
        if args.verbose:
            iterable1 = tqdm(iterable1, desc='Iterating over detectors')
        for detector_group_name, in_detector_group in iterable1:
            out_detector_group = outfile.create_group(detector_group_name)
            copy_attrs(in_detector_group, out_detector_group)
            iterable2 = in_detector_group.items()
            if args.verbose:
                iterable2 = tqdm(iterable2, desc='Iterating over segments', leave=False)
            for segment_name, in_segment in iterable2:
                ...
                out_segment = out_detector_group.create_dataset(segment_name, data=caster(segment_white_data), compression='gzip', compression_opts=9, shuffle=True)
